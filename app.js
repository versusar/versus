...
// FIX: State is declared but not initialized until initApp() runs
let state;
  ...
  if (step === 1) {
    ...
    $$('.interest-tag').forEach(tag => {
      tag.addEventListener('click', () => {
        ...
        if (selected.has(cat)) {
          selected.delete(cat);
          tag.style.background = 'transparent'; tag.style.borderColor = 'rgba(255,255,255,.3)';
        } else {
          selected.add(cat);
          tag.style.background = 'rgba(255,255,255,.25)'; tag.style.borderColor = '#fff';
        }
      });
    });
  }
...
function handleShare() {
  ...
  try {
    if (navigator.share) {
      navigator.share({ title: 'Versus ⚡', text, url: APP_URL }).then(afterShare).catch(() => { fallbackCopy(text); afterShare(); });
    } else { fallbackCopy(text); afterShare(); }
  } catch (e) { fallbackCopy(text); afterShare(); }
}
...
// FIX: Wait for DOM and external scripts to be ready before calling init
